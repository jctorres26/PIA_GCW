  ...
  />
	...
	function render() {
		...
		if(itemHasSpawned==true && hasItem == false){
			...
			if(itemP1Collision || itemP2Collision){
				console.log("item");
				hasItem = true;
				if(jugadores[0].isAlive){
				jugadores[0].health = jugadores[0].health + 1;}
				if(jugadores[1].isAlive){
				jugadores[1].health = jugadores[1].health + 1;}
	...
	function spawnItem(){
		if(killedEnemies == 1 && itemHasSpawned == false){
			itemHasSpawned = true;
			loader.load('assets/medkit.fbx',function(rayo){
	...
	function setupScene() {		
		...
		loader.load('assets/runInPlace.fbx',function(redWizardRunning){
			...
			loader.load('assets/escenario3test.fbx',function(escenario){

				escenario.position.z = -10;
				escenario.position.x = -92;
...
<div id="id01" class="modal">
  ...
  <form class="modal-content" action="./name.php">
    <div class="container">
      <h1>Has completado el ultimo nivel!</h1>
      <p>Presiona el boton para ver tu tiempo:</p>
